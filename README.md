...
- **메인 홈**
  - 인기 스타일(좋아요 Top N) 조회
  - 사용자 BodyShape 기반 유사 스타일 추천 :contentReference[oaicite:10]{index=10}
- **상품**
  - 전체 상품 / 카테고리별 조회 / 상세 조회 :contentReference[oaicite:11]{index=11}
  - 상세에서 리뷰 요약(평점 평균/리뷰 수/미리보기) 및 피팅 이미지 리스트 제공
- **검색**
  - 키워드 기반 검색 (HIDDEN 제외, 할인율 정규화 및 할인 가격 계산) :contentReference[oaicite:12]{index=12} :contentReference[oaicite:13]{index=13}
- **리뷰**
  - 리뷰 작성(multipart: dto + images) 및 S3 업로드
  - 리뷰 작성 시 적립금 지급(결제 금액 기준 5%) :contentReference[oaicite:14]{index=14} :contentReference[oaicite:15]{index=15}
- **반품**
  - 반품 신청 / 내역 조회 / 상세 조회 :contentReference[oaicite:16]{index=16}
- **AI 피팅**
...
- **상품 관리**
  - 상품 생성 시:
    - productCode 자동 생성
    - 상품 이미지 **S3 업로드**
    - 동일 파일명을 **Comfy 서버(/upload/image)**로도 전달하여 AI 파이프라인 입력 동기화 :contentReference[oaicite:17]{index=17}
- **반품 관리**
  - 전체 반품 목록/상세 조회
  - 상태 변경(PATCH) 및 반려 사유 처리 :contentReference[oaicite:18]{index=18}
  - 상태 COMPLETED 시:
    - 주문 금액에 따라 **전액/부분 환불**
    - 사용 포인트 반환 + 포인트 히스토리 적재
    - 재고 복구(상품 아이템 stock + 1) :contentReference[oaicite:19]{index=19}
...
- 같은 파일명으로 Comfy 서버에도 업로드(`/upload/image`)합니다. :contentReference[oaicite:20]{index=20}

### 검색/가격 계산
- 할인율이 `10`(%) 형태로 저장되거나 `0.1` 형태일 수 있어 정규화 후 할인 가격을 계산합니다. :contentReference[oaicite:21]{index=21}

### 리뷰 적립금 + 이미지 S3 업로드
- 리뷰 작성 시 주문 아이템 금액 기준으로 적립금을 지급하고,
- 리뷰 이미지는 S3에 업로드하여 URL을 저장합니다. :contentReference[oaicite:22]{index=22}

### 반품 처리(환불/포인트/재고)
- 반품 상태 COMPLETED 시:
  - 주문 금액에 따라 전액/부분 환불 분기
  - 사용 포인트 반환 및 히스토리 기록
  - 재고 복구 :contentReference[oaicite:23]{index=23}
...
### Main Home
- `GET /api/v1/main/popular-styles` : 인기 스타일(좋아요 Top) :contentReference[oaicite:24]{index=24}  
- `GET /api/v1/main/similar-styles` : 사용자 BodyShape 기반 추천 :contentReference[oaicite:25]{index=25}

### Products
- `GET /api/v1/products` : 전체 상품 :contentReference[oaicite:26]{index=26}  
- `GET /api/v1/products/category/{categoryId}` : 카테고리별 :contentReference[oaicite:27]{index=27}  
- `GET /api/v1/products/{productId}` : 상세 :contentReference[oaicite:28]{index=28}  

### Search
- `GET /api/v1/search?keyword=...` : 검색 :contentReference[oaicite:29]{index=29}

### Reviews
- `POST /api/v1/reviews` (multipart: dto + images) : 리뷰 작성 :contentReference[oaicite:30]{index=30}  
- `GET /api/v1/reviews` : 내 리뷰 조회 :contentReference[oaicite:31]{index=31}  

### Returns
- `POST /api/v1/return` : 반품 신청 :contentReference[oaicite:32]{index=32}  
- `GET /api/v1/return/my` : 내 반품 내역 :contentReference[oaicite:33]{index=33}  
- `PATCH /api/v1/admin/returns/{returnId}/status/{status}?reason=` : (관리자) 반품 상태 변경 :contentReference[oaicite:34]{index=34}  
